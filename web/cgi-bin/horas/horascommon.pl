...
sub getrank {
  ...
  if ($hora =~ /(vespera|completorium)/i) {
    ...
    if ( ($version =~ /1955/ && $crank[2] < 5)
      || ($version =~ /1960|Newcal|Monastic/i && $version !~ /Cistercian/i && $crank[2] < (($csaint{Rule} =~ /Festum Domini/i && $dayofweek == 6) ? 5 : 6)))
    {
      $crank = '';
      @crank = ();
    }
...
sub climit1960 {
  my $c = shift;
  if (!$c) { return 0; }
  if ($version =~ /Cistercian/i ) { return 1; } # so far all the Cistercian comm. are allowed in this function
